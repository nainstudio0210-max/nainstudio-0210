...
export default function AIArchVizPage() {
  ...
  const handleTouchEnd = () => {
    ...
    if (distance > 50) goLightboxNext()
    else if (distance < -50) goLightboxPrev()
    setTouchStartX(null)
    setTouchEndX(null)
  }
  ...
  const menuVariants: Variants = {
    ...
    visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.3, ease: "easeOut", when: "beforeChildren", staggerChildren: 0.08 } },
  ...
  return (
    <div className="relative min-h-screen bg-black text-white">
      <div className="fixed top-5 right-5 md:top-8 md:right-8 z-[200]" ref={menuRef}>
        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="flex items-center justify-center text-white/80 hover:text-white transition-colors focus:outline-none" aria-label="Open Menu">
          {isMenuOpen ? <XIcon className="w-7 h-7 md:w-8 md:h-8" /> : <Menu className="w-7 h-7 md:w-8 md:h-8" />}
        </button>

        <AnimatePresence>
          {isMenuOpen && (
            <motion.div className="absolute right-0 mt-3 w-44 md:w-52 bg-[#111] border border-white/10 rounded-lg shadow-2xl flex flex-col py-2 overflow-hidden" variants={menuVariants} initial="hidden" animate="visible" exit="exit">
              <motion.div variants={itemVariants}><Link href="/" onClick={() => setIsMenuOpen(false)} className="block px-5 py-3 text-sm md:text-base text-white/70 hover:text-white hover:bg-white/10 transition-colors">Home</Link></motion.div>
              <motion.div variants={itemVariants}><Link href="/work" onClick={() => setIsMenuOpen(false)} className="block px-5 py-3 text-sm md:text-base text-white font-medium bg-white/10 transition-colors">Works</Link></motion.div>
              <motion.div variants={itemVariants}><Link href="/contact" onClick={() => setIsMenuOpen(false)} className="block px-5 py-3 text-sm md:text-base text-white/70 hover:text-white hover:bg-white/10 transition-colors">Contact</Link></motion.div>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
        ...
        <nav className="w-full flex flex-col text-[10px] md:text-sm tracking-wide mt-4 md:mt-5">
          ...
          <div className="flex flex-col items-center w-full">
            ...
            <div className="flex flex-col items-start gap-2.5 md:gap-3 pl-6 md:pl-10">
              ...
              <Link href="/ai-archviz" className="text-white hover:text-white flex items-center gap-2 group">
      ...
      <AnimatePresence>
        {active && activeIndex != null && (
          <motion.div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={close}>
            <motion.div className="relative w-[90%] h-[85%] md:w-[85%] md:h-[90%] rounded-xl overflow-hidden bg-zinc-900/50 shadow-2xl" initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.98, opacity: 0 }} onClick={(e) => e.stopPropagation()}>
                ...
                {active.type === "gallery" ? (
                  <div className="w-full flex flex-col items-center gap-6">
                    {flatGalleryItems.map((content, index) => (
                      <div key={index} onClick={() => { if (typeof window !== 'undefined' && window.innerWidth < 768 && content.type === 'image') setLightboxIndex(index) }} className="relative w-full max-w-5xl shadow-2xl bg-black md:cursor-default cursor-pointer">
                        {content.type === "youtube" ? (
                          ...
                          <div className="relative w-full">
                            <Image src={content.src || ""} alt={`Gallery Item ${index}`} width={0} height={0} sizes="100vw" className="w-full h-auto object-contain rounded-sm select-none" style={{ WebkitTouchCallout: 'none' }} draggable={false} priority={index === 0} />
                            <div className="absolute bottom-3 right-3 md:hidden bg-black/60 p-2 rounded-full pointer-events-none"><ZoomIn className="w-5 h-5 text-white/90" /></div>
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  <div className="w-full h-full relative cursor-pointer md:cursor-default flex items-center justify-center p-4" onClick={() => { if (typeof window !== 'undefined' && window.innerWidth < 768) setLightboxIndex(0) }}>
                    ...
                    <div className="absolute bottom-6 right-6 md:hidden bg-black/60 p-3 rounded-full pointer-events-none"><ZoomIn className="w-6 h-6 text-white/90" /></div>
                  </div>
                ) : active.type === "youtube" ? (
      ...
      <AnimatePresence>
        {lightboxIndex !== null && flatGalleryItems.length > 0 && (
          <motion.div className="fixed inset-0 z-[100] bg-black flex flex-col md:hidden touch-none" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: "tween", duration: 0.2 }}>
            <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center z-[110] bg-gradient-to-b from-black/80 to-transparent pt-8">
              <span className="text-white/90 font-semibold tracking-widest pl-2">{lightboxIndex + 1} <span className="text-white/50 text-sm">/ {flatGalleryItems.length}</span></span>
              <button onClick={() => setLightboxIndex(null)} className="p-2 bg-white/10 rounded-full text-white"><XIcon size={24} /></button>
            </div>
            <div className="w-full h-full relative flex items-center justify-center" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
            ...
            {flatGalleryItems.length > 1 && (
              <div className="absolute bottom-10 left-0 right-0 flex justify-center gap-2 z-[110]">
                {flatGalleryItems.map((_, idx) => <div key={idx} className={`w-1.5 h-1.5 rounded-full ${idx === lightboxIndex ? 'bg-[#e85d22]' : 'bg-white/30'}`} />)}
              </div>
            )}
          </motion.div>
        )}
...
function Tile({ item, span, onOpen, priority = false }: { item: MediaItem, span: string, onOpen: () => void, priority?: boolean }) {
  const posterSrc = item.poster || (item.galleryContents && item.galleryContents.find(c => c.type === "image")?.src) || (item.images && item.images[0]) || item.src || '';
  return (
    <div className={["group relative cursor-pointer overflow-hidden rounded-md md:rounded-lg bg-zinc-900 select-none", "col-span-2 row-span-2", span].join(" ")} onClick={onOpen} onContextMenu={(e) => e.preventDefault()}>
      <div className="absolute inset-0">
        {(item.type === "image" || item.type === "gallery") && posterSrc ? (
          <Image src={posterSrc} alt={item.title || ""} fill sizes="(max-width: 768px) 100vw, 33vw" priority={priority} draggable={false} className="object-cover transition-transform duration-500 group-hover:scale-[1.03] select-none" style={{ WebkitTouchCallout: 'none' }} />
        ) : (
          <>
            {item.poster ? <Image src={item.poster} alt={item.title || ""} fill sizes="(max-width: 768px) 100vw, 33vw" draggable={false} className="object-cover select-none" style={{ WebkitTouchCallout: 'none' }} priority={priority} /> : <div className="absolute inset-0 bg-zinc-800" />}
            <div className="absolute inset-0 grid place-items-center">
              <div className="rounded-full bg-black/55 border border-white/30 p-3">
                {item.type === "youtube" ? <Maximize className="w-6 h-6 text-white" /> : <Play className="w-6 h-6 text-white" />}
              </div>
            </div>
          </>
        )}
      ...
      {(item.title || item.caption) && (
        <>
          ...
          <div className="absolute left-3 right-3 bottom-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            {item.title && <div className="text-sm md:text-base font-medium flex items-center gap-2">{item.title}{item.type === "gallery" && <Layers className="w-3 h-3 text-white/70" />}</div>}
