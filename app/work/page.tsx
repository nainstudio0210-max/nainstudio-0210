...
// 2. MediaItem 타입 정의 (span은 이제 필수값처럼 다룹니다)
type MediaItem = {
  id: string
  ...
  caption?: string
  poster?: string
  span: string // ★ 여기서 span을 직접 지정하므로 필수 속성으로 생각하면 편합니다
}
...
export default function WorkPage() {
  ...
  // ▼▼▼ 여기에 모든 아이템의 크기(span)를 직접 고정했습니다. ▼▼▼
  // 이제 순서를 바꿔도 이 span 값 때문에 모양이 유지됩니다.
        ...
        >
          {items.map((item, i) => (
            <Tile
              key={item.id}
              item={item}
              span={item.span} // ★ 이제 계산하지 않고 아이템에 적힌 값을 그대로 씁니다
              onOpen={() => openAt(i)}
              priority={i < 6}
            />
          ))}
